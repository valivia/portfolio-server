...
class getAsssetService {
    ...
    public getFile(folder: string, fileName: string, req: Request, res: Response, next: NextFunction): void {
        const file: string = this.validateFilePath(folder, fileName);

        // Get mime type.
        const type: string = mime[path.extname(file).slice(1)] || "text/plain";

        res.set("Cache-control", `public, max-age=${60 * 60 * 24 * 32}`);
...
export default getAsssetService;
