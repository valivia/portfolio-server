...
class ProjectController implements Controller {
    ...
    private get_project = async (
        req: Request,
        res: Response,
        next: NextFunction,
    ) => {
        this.getService.main(req, res, this.db).catch((e: Error) => {
            next(e);
        });
    };

    private get_projects = async (
        req: Request,
        res: Response,
        next: NextFunction,
    ) => {
        GetProjectsService(req, res, this.db).catch((e: Error) => {
            next(e);
        });
    };

    private post_project = (
        req: Request,
        res: Response,
        next: NextFunction,
    ) => {
        postProject(req, res, this.db).catch((e: Error) => {
            next(e);
        });
    };

    private patch_project = (
        req: Request,
        res: Response,
        next: NextFunction,
    ) => {
    ...
    private delete_project = (
        ...
        deleteProject(req, res, this.db).catch((e: Error) => {
            next(e);
        });
    };
}
